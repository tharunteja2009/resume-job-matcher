...
def insert_candidate_to_mongo(data: str) -> str:
    ...
    try:
        ...
        try:
            ...
            return f"✅ Successfully inserted candidate data for {data_dict.get('candidate_name', 'Unknown')}"
        except errors.DuplicateKeyError:
            return f"⚠️ Candidate {data_dict.get('candidate_name', 'Unknown')} already exists. Skipping insertion."

    except json.JSONDecodeError as e:
        error_msg = f"Error: JSON syntax error - {str(e)}. Check JSON structure around character position mentioned in error."
        return error_msg
